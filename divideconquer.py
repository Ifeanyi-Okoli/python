...
def helper(lst, group_len, i):
    if lst[i] == lst[i+1]:
        temp = lst[i+1]
        lst[i+1] = lst[i+2]
        lst[i+2] = temp
        helper(lst, group_len, i+1)
    for i in range(0, len(lst)-group_len, group_len):
        #check if consecutive
        if lst[i] != lst[i+1] - 1:
            return False
        elif lst[i] + 1 != lst[i+ group_len - 1]:
            return False
    return True
...
def consecutive_nums(lst, group_len):
    #sort list
    lst.sort()
    if lst and group_len == 1:
        return True
    if lst and len(lst) % group_len == 0:
